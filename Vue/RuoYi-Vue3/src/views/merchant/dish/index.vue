    ...
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="售卖状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择售卖状态" clearable>
          <el-option v-for="dict in dish_status" :key="dict.value" :label="dict.label" :value="dict.value" />
        </el-select>
      </el-form-item>
      <el-form-item>
      ...
      <el-form ref="dishRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="售卖状态" prop="status">
          <el-select v-model="form.status" placeholder="请选择售卖状态">
            <el-option v-for="dict in dish_status" :key="dict.value" :label="dict.label"
        ...
        <el-table :data="dishFlavorList" :row-class-name="rowDishFlavorIndex"
          ...
          <el-table-column label="口味名称" prop="name" width="150">
            <template #default="scope">
              ...
              <el-select v-model="scope.row.name" placeholder="请选择口味名称" @change="changeFlavorName(scope.row)">
                <el-option v-for="dishFlavor in dishFlavorListSlect" :key="dishFlavor.name" :label="dishFlavor.name"
          ...
          <el-table-column label="口味列表" prop="value" width="350">
            <template #default="scope">
              ...
              <el-select v-model="scope.row.value" placeholder="请选择口味列表" multiple @focus="FocusFlavorName(scope.row)" style="width: 90%;">
...
// ---------------------------------------------
// 定义口味名称和口味列表静态数据
const dishFlavorListSlect = ref([
  { name: "辣度", value: ["不辣", "微辣", "中辣", "重辣"] },
  { name: "忌口", value: ["不要葱", "不要蒜", "不要香菜", "不要辣"] },
])
// 定义一个存储当前选中口味列表的数据
const checkValueList = ref([]);
// 定义改变口味名称时更新当前当前选中的口味列表
function changeFlavorName(row) {
  // 根据选中的name去查找静态数据的value
  // 清空当前行的value
  row.value = [];
  checkValueList.value = dishFlavorListSlect.value.find(item => item.name === row.name).value;
}
// 定义口列表获取焦点时候，更新当前当前选中的口味列表
